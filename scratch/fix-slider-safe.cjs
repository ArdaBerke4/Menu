...
function processMenu() {
  ...
  const oldFontDef = `const getFontSizeClasses = (size?: string) => {
  switch (size) {
  ...
  if(c.includes(oldFontDef)) {
    c = c.replace(oldFontDef, newFontDef);
  } else {
    console.error("Failed to find Menu font def");
  }
  ...
  const catOld = `className={\`text-2xl font-black tracking-widest \${fs.cat}\`}`;
  const catNew = `className="font-black tracking-widest"`;
  c = c.replace(catOld, catNew);
...
function processSettings() {
  ...
  // Replace font styles
  const oldFontDef = `export const getFontSizeClasses = (size?: string) => {
    switch (size) {
  ...
  if(c.includes(oldFontDef)) {
    ...
    console.error("Failed to find Settings font def");
  }
  ...
  c = c.replace(
    ...
    `className="font-bold leading-tight line-clamp-2" style={getDynamicFontStyles(fontSize).product}`
  );
  c = c.replace(
    /className=\{`font-bold mb-1 \$\{getDynamicFontStyles\(fontSize\)\.product\}`\}/g,
    `className="font-bold mb-1 line-clamp-2" style={getDynamicFontStyles(fontSize).product}`
  );
  c = c.replace(
  ...
  c = c.replace(
    ...
    `className="font-bold flex items-center" style={getDynamicFontStyles(fontSize).price}`
  );
  c = c.replace(
    /className=\{`font-bold mt-auto \$\{getDynamicFontStyles\(fontSize\)\.price\}`\}/g,
    `className="font-bold mt-auto" style={getDynamicFontStyles(fontSize).price}`
  );
