      ...
      <div className="w-[450px] shrink-0 space-y-8">
        <div className="bg-admin-bg border-4 border-admin-border shadow-admin-pixel p-6">
          ...
          <form onSubmit={handleCreateCategory} className="flex gap-4">
            <input type="text" required value={categoryName} onChange={(e) => setCategoryName(e.target.value)} placeholder="Örn: Kahveler..." className="flex-1 px-4 py-2 border-2 border-admin-border bg-admin-surface focus:outline-none" />
        ...
        <div className="bg-admin-bg border-4 border-admin-border shadow-admin-pixel p-6">
          ...
          <div className="mt-4 flex flex-col gap-1 text-xs font-bold opacity-60">
            ...
            <table className="w-full border border-admin-border text-left bg-admin-surface mt-1">
              <thead className="border-b border-admin-border"><tr><th className="px-2 py-1 border-r border-admin-border">Kategori</th><th className="px-2 py-1 border-r border-admin-border">Ürün Adı</th><th className="px-2 py-1">Fiyat</th></tr></thead>
              <tbody><tr><td className="px-2 py-1 border-r border-admin-border">Tatlılar</td><td className="px-2 py-1 border-r border-admin-border">Sufle</td><td className="px-2 py-1">150</td></tr></tbody>
            </table>
          </div>
        </div>
        ...
        {sortedCategories.length > 0 && (
          <div className="bg-admin-bg border-4 border-admin-border shadow-admin-pixel p-6">
              <>
                <h2 className="text-2xl font-bold mb-4 uppercase text-admin-text border-b-2 border-admin-border pb-2">Kategori Sıralaması</h2>
        ...
        <div className={`bg-admin-bg border-4 border-admin-border shadow-admin-pixel p-6 transition-colors duration-300 ${editingProductId ? 'bg-admin-primary-faint' : ''}`}>
          ...
          <form onSubmit={handleSubmitProduct} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" required value={productName} onChange={(e) => setProductName(e.target.value)} placeholder="Örn: Flat White" className="w-full px-4 py-2 border-2 border-admin-border bg-admin-surface focus:outline-none" />
            </div>
            <div>
              <label className="block font-bold mb-1">Açıklama</label>
              <textarea rows={2} value={productDesc} onChange={(e) => setProductDesc(e.target.value)} placeholder="Kısa açıklama..." className="w-full px-4 py-2 border-2 border-admin-border bg-admin-surface focus:outline-none resize-none" />
            </div>
            <div>
            ...
            <div className="border-2 border-admin-border p-3 bg-admin-surface space-y-3">
              <div className="flex justify-between items-center">
                <label className="font-bold">Ekstralar / Seçenekler</label>
              ...
              {productOptions.length === 0 ? (
                <p className="text-xs opacity-50 italic">Ürüne ait ekstra seçenek yok.</p>
              ) : (
                  ...
                  {productOptions.map((opt: any, optIdx: number) => (
                    <div key={optIdx} className="border border-admin-border/30 p-2 bg-gray-50 relative group">
                      ...
                      <div className="flex gap-2 mb-2 pr-8">
                        <input type="text" placeholder="Grup Adı (Örn: Süt Seçimi)" value={opt.name} onChange={(e) => {
                      ...
                      <div className="space-y-2 ml-2 border-l-2 border-brand-light pl-2">
                        {opt.choices.map((choice: any, choiceIdx: number) => (
                          <div key={choiceIdx} className="flex gap-2">
                            <input type="text" placeholder="Seçenek (Örn: Badem Sütü)" value={choice.name} onChange={(e) => {
                              const newOpts = [...productOptions];
                            ...
                            <div className="flex items-center border border-admin-border bg-admin-surface">
                              ...
                              <span className="px-2 text-xs bg-gray-100 border-l border-admin-border">₺</span>
                            </div>
                            <button type="button" onClick={() => {
                              const newOpts = [...productOptions];
                              newOpts[optIdx].choices = newOpts[optIdx].choices.filter((_: any, c: number) => c !== choiceIdx);
                              setProductOptions(newOpts);
                            }} className="text-red-500 font-bold px-2 hover:bg-red-100">✕</button>
                          </div>
                        ))}
                        <button type="button" onClick={() => {
                          const newOpts = [...productOptions];
                          ...
                          setProductOptions(newOpts);
                        }} className="text-xs font-bold text-brand hover:underline mt-1 block">+ Yeni Seçenek</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div>
              ...
              <input type="text" inputMode="decimal" required value={productPrice} onChange={(e) => setProductPrice(e.target.value)} placeholder="Örn: 120" className="w-full px-4 py-2 border-2 border-admin-border bg-admin-surface focus:outline-none" />
            </div>
            <div className="flex gap-4 pt-2">
              ...
              {editingProductId && <button type="button" onClick={resetProductForm} className="bg-gray-300 text-admin-text border-2 border-admin-border px-4 py-3 text-lg shadow-admin-pixel hover:bg-gray-400">İPTAL</button>}
            </div>
          </form>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col space-y-6">
        <div className="bg-admin-bg border-4 border-admin-border shadow-admin-pixel p-4 space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span className="font-bold text-admin-text shrink-0">FİYAT SIRALA:</span>
            <div className="flex border-2 border-admin-border">
              <button type="button" onClick={() => setSortByPrice(sortByPrice === 'asc' ? null : 'asc')} className={`px-4 py-2 font-bold transition-colors ${sortByPrice === 'asc' ? 'bg-brand text-surface' : 'bg-brand-light text-admin-text hover:bg-admin-surface'}`}>Artan ↑</button>
              <button type="button" onClick={() => setSortByPrice(sortByPrice === 'desc' ? null : 'desc')} className={`px-4 py-2 font-bold border-l-2 border-admin-border transition-colors ${sortByPrice === 'desc' ? 'bg-brand text-surface' : 'bg-brand-light text-admin-text hover:bg-admin-surface'}`}>Azalan ↓</button>
            </div>
            {sortByPrice && (
              <>
                <button type="button" onClick={() => setSortByPrice(null)} className="px-3 py-2 bg-gray-300 text-admin-text border-2 border-admin-border font-bold hover:bg-gray-400 text-sm">Sıfırla ✕</button>
                <span className="text-sm font-bold text-[#5b7a57] bg-admin-primary-faint border border-admin-primary px-3 py-1">{sortByPrice === 'asc' ? '💰 Ucuzdan pahalıya' : '💎 Pahalıdan ucuya'}</span>
              </>
            )}
          </div>
            ...
            <div className="bg-admin-primary-faint border-2 border-admin-primary p-3 space-y-3">
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex gap-3">
                  <button type="button" onClick={() => { setBulkMode(bulkMode === 'increase' ? null : 'increase'); setBulkValue(''); }} className={`font-bold border-2 border-admin-border px-4 py-1 shadow-admin-pixel-sm hover:scale-105 transition-transform ${bulkMode === 'increase' ? 'bg-brand text-surface' : 'bg-admin-primary text-surface'}`}>+ Toplu Zam</button>
              ...
              {bulkMode && (
                <div className="flex flex-wrap items-center gap-3 bg-admin-surface border-2 border-admin-border p-3">
                  ...
                  <div className="flex border-2 border-admin-border">
                    <button type="button" onClick={() => setBulkValueType('percent')} className={`px-3 py-2 font-bold ${bulkValueType === 'percent' ? 'bg-brand text-surface' : 'bg-admin-surface text-admin-text'}`}>Yüzde (%)</button>
                    <button type="button" onClick={() => setBulkValueType('fixed')} className={`px-3 py-2 font-bold border-l-2 border-admin-border ${bulkValueType === 'fixed' ? 'bg-brand text-surface' : 'bg-admin-surface text-admin-text'}`}>Sabit Tutar (₺)</button>
                  </div>
                  <div className="flex items-center border-2 border-admin-border">
                    ...
                    <span className="px-3 py-2 bg-brand-light border-l-2 border-admin-border font-bold">{bulkValueType === 'percent' ? '%' : '₺'}</span>
                  </div>
                  <button type="button" onClick={handleApplyBulkAction} disabled={loading || !bulkValue} className="bg-admin-primary text-surface font-bold border-2 border-admin-border px-5 py-2 shadow-admin-pixel-sm hover:scale-105 disabled:opacity-50">{loading ? 'UYGULANIYOR...' : 'UYGULA'}</button>
                  <button type="button" onClick={() => { setBulkMode(null); setBulkValue(''); }} className="bg-gray-300 text-admin-text font-bold border-2 border-admin-border px-3 py-2 hover:bg-gray-400">İPTAL</button>
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-admin-surface border-4 border-admin-border shadow-admin-pixel overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto max-h-[600px] p-4 space-y-3">
            {filteredProducts.length === 0 ? (
              <div className="text-center py-10 font-bold text-admin-text/50">Bu kriterlere uygun ürün bulunamadı.</div>
            ) : (
